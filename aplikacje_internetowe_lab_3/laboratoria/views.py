...
from .models import calculation_database
...
def lab_1(request):
    if 'logged_in' in request.session and request.session['logged_in'] == True:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if "save" in request.POST:
                    new_post = calculation_database(amount=loan_amount, percentage=interest_rate, years=repayment_period, monthly_payment=monthly_payment, total_payment=total_payment)
                    new_post.save()
        ...
        all_entries = calculation_database.objects.all()
            
        return render(request, 'lab_1.html', {'form': form, "summary_data": summary_data, "all_entries": all_entries})
    else:
